...
// 加载网页
- (void)loadWeb {
    [webView loadRequest:[NSURLRequest requestWithURL:[NSURL URLWithString:@"http://192.168.3.31:8080/"]]];
...
- (void)savePrompt:(NSString *)prompt {
    ...
    InfoAlert * v = [[InfoAlert alloc]initWithTitle:promptDic[@"action"] msgString:[self convertToJsonData:promptDic] buttonString:promptDic[@"action"] leftString:@""];
    v.callback = ^(NSString *string) {
        if (promptDic[@"action"]) {
            // login
            if ([promptDic[@"action"] isEqualToString:@"login"]) {
                self.isLogin = YES;
                [self loginRequest:promptDic];
                // invoke
            }else if ([promptDic[@"action"] isEqualToString:@"invoke"]){
                self.isLogin = NO;
                [self invokeTransactionRequest:promptDic];
                // getAccount
            }else if ([promptDic[@"action"] isEqualToString:@"getAccount"]){
                [self getAccount:promptDic];
            }
        }
    };
    [v show];
...
-(void)getAccount:(NSDictionary*)resultDic{
    ...
    NSString *jsStr = [NSString stringWithFormat:@"javascript:emitMessage('%@')",base64String ];
...
- (void)handlePrompt:(NSString *)prompt{
    ...
    if ([prompt hasPrefix:@"decryptEncryptedPrivateKey"]) {
            ...
            *pop = [[MGPopController alloc] initWithTitle:Localized(@"PASSWORDERROR") message:nil image:nil];
            ...
            if (promptDic[@"action"]) {
                if ([promptDic[@"action"] isEqualToString:@"login"]) {
                    ...
                    // 对 message 签名
                    NSDictionary *params = promptDic[@"params"];
                    ...
                    NSString *str = [self convertToJsonData:promptDic];
        ...
        v.callback = ^(NSString *string) {
            _hub=[ToastUtil showMessage:@"" toView:nil];
            NSString* jsStr  =  [NSString stringWithFormat:@"Ont.SDK.sendTransaction('%@','sendTransaction')",hashString];
